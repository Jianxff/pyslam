...
namespace PLSLAM
{
    namespace data
    {
        ...
        void landmark::update_normal_and_depth()
        {
            ...
            const auto scale_level = ref_keyfrm->undist_keypts_.at(observations.at(ref_keyfrm)).octave; // which level of image pyramid the keypoint is extracted
            const auto scale_factor = ref_keyfrm->scale_factors_.at(scale_level);                       // the scale factor = pow(1.2, scale_level) corresponding to the level of image pyramid
            const auto num_scale_levels = ref_keyfrm->num_scale_levels_;                                // = 8
