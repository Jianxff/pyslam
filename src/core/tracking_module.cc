...
#include "core/feature/orb_extractor.h"
#include "core/match/projection.h"
#include "core/module/local_map_updater.h"
...
namespace PLSLAM
{
    ...
    Mat44_t tracking_module::track_monocular_image(const cv::Mat &img, const double timestamp, const cv::Mat &mask)
    {
        ...
        if (tracking_state_ == tracker_state_t::Initializing)
        {
            ...
            {
                // FW: pass the line_extractor to frame.h (case of Initializing)
                curr_frm_ = data::frame(img_gray_, timestamp, ini_extractor_left_, bow_vocab_,
                                        _line_extractor,
                                        camera_, cfg_->true_depth_thr_, mask);
            }
    ...
    void tracking_module::track()
    {
        ...
        else
        {
            ...
            // - robust_match_based_track() ~ 40ms (seems too slow to be useful), this one added by PLSLAM authors
