...
#include "core/data/frame.h"
#include "core/data/keyframe.h"
#include "core/data/landmark.h"
#include "core/match/bow_tree.h"
#include "core/match/angle_checker.h"
    ...
    namespace match
    {
        ...
        unsigned int bow_tree::match_frame_and_keyframe(data::keyframe *keyfrm, data::frame &frm, std::vector<data::landmark *> &matched_lms_in_frm) const
        {
            ...
            while (keyfrm_itr != kryfrm_end && frm_itr != frm_end)
            {
                ...
                {
                    // If the node number (first) of the BoW tree matches,
                    // To actually make sure that it corresponds bring a feature point index (second)
                    ...
                    {
                        // If the feature point of keyfrm_idx and the 3D point do not correspond, it will be passed through.
                ...
                {
                    // Since the node number of keyfrm_itr is smaller, we recommend the iterator keyfrm_itr until the node number matches.
                    keyfrm_itr = keyfrm->bow_feat_vec_.lower_bound(frm_itr->first);
                }
                else
                {
                    // Since the node number of frm_itr is smaller, we recommend the iterator frm_itr until the node number matches.
        ...
        unsigned int bow_tree::match_keyframes(data::keyframe *keyfrm_1, data::keyframe *keyfrm_2, std::vector<data::landmark *> &matched_lms_in_keyfrm_1) const
        {
            ...
            // Of the feature points of keyframe2, set to true if it corresponds to the feature point of keyfram1.
            // NOTE: size matches the feature point of keyframe2
            ...
            while (itr_1 != itr_1_end && itr_2 != itr_2_end)
            {
                ...
                {
                    // If the node number (first) of the BoW tree matches,
                    // Actually bring the feature point index (second) and check if it corresponds
                    const auto &keyfrm_1_indices = itr_1->second;
                    const auto &keyfrm_2_indices = itr_2->second;

                    for (const auto idx_1 : keyfrm_1_indices)
                    {
                        // If the feature point of keyfrm_1 and the 3D point do not correspond, it will be passed through.
                        ...
                        {
                            // If the feature point of keyfrm_2 does not correspond to the 3D point, it will be passed through.
                        ...
                        // Record the correspondence information
                        // idx_1 of keyframe1 and best_idx_2 of keyframe2 correspond
                        matched_lms_in_keyfrm_1.at(idx_1) = keyfrm_2_lms.at(best_idx_2);
                        // The best_idx_2 of keyframe2 already corresponds to the feature point of keyframe1
