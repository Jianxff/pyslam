...
namespace pangolin_viewer
{
    ...
    viewer::viewer(const std::shared_ptr<PLSLAM::config> &cfg, PLSLAM::system *system,
                   const std::shared_ptr<PLSLAM::publish::frame_publisher> &frame_publisher,
                   const std::shared_ptr<PLSLAM::publish::map_publisher> &map_publisher)
    ...
    void viewer::draw_keyframes()
    {
        ...
        std::vector<PLSLAM::data::keyframe *> keyfrms;
        ...
        if (*menu_show_graph_)
        {
            ...
            const auto draw_edge = [](const PLSLAM::Vec3_t &cam_center_1, const PLSLAM::Vec3_t &cam_center_2)
            {
            ...
            for (const auto keyfrm : keyfrms)
            {
                ...
                const PLSLAM::Vec3_t cam_center_1 = keyfrm->get_cam_center();
                ...
                if (!covisibilities.empty())
                {
                    for (const auto covisibility : covisibilities)
                    {
                        ...
                        const PLSLAM::Vec3_t cam_center_2 = covisibility->get_cam_center();
                ...
                if (spanning_parent)
                {
                    const PLSLAM::Vec3_t cam_center_2 = spanning_parent->get_cam_center();
                    draw_edge(cam_center_1, cam_center_2);
                }
                ...
                for (const auto loop_edge : loop_edges)
                {
                    ...
                    const PLSLAM::Vec3_t cam_center_2 = loop_edge->get_cam_center();
                    draw_edge(cam_center_1, cam_center_2);
                }
            }
    ...
    void viewer::draw_landmarks()
    {
        ...
        std::vector<PLSLAM::data::landmark *> landmarks;
    ...
    // FW:
    void viewer::draw_dense_pointcloud(PLSLAM::data::keyframe *kf)
    {
        ...
        auto camera = static_cast<PLSLAM::camera::perspective *>(camera_);
    ...
    void viewer::draw_landmarks_line()
    {
        ...
        std::vector<PLSLAM::data::Line *> landmarks_line;
        ...
        for (const auto lm : landmarks_line)
        {
            ...
            PLSLAM::Vec6_t pos_w = lm->get_pos_in_world();
    ...
    void viewer::draw_camera(const PLSLAM::Mat44_t &cam_pose_wc, const float width) const
    {
        glPushMatrix();
