...
#include "core/util/stereo_rectifier.h"
#include "core/util/image_converter.h"
...
void mono_tracking(const std::shared_ptr<PLSLAM::config> &cfg,
                   ...
                   const bool eval_log, const std::string &map_db_path, const bool equal_hist)
{
    ...
    // build a SLAM system
    // FW:
    // - the 3rd parameter: b_use_line_tracking (optional)
    PLSLAM::system SLAM(cfg, vocab_file_path, true);
    ...
    std::thread thread([&]()
    {
        for (unsigned int i = 0; i < frames.size(); ++i)
        {
            ...
            if (equal_hist)
            {
                ...
                PLSLAM::util::equalize_histogram(img);
            }
            else
            {
...
void stereo_tracking(const std::shared_ptr<PLSLAM::config> &cfg,
                     const std::string &vocab_file_path, const std::string &sequence_dir_path,
                     const unsigned int frame_skip, const bool no_sleep, const bool auto_term,
                     const bool eval_log, const std::string &map_db_path, const bool equal_hist)
{
    ...
    const PLSLAM::util::stereo_rectifier rectifier(cfg);
    ...
    std::thread thread([&]()
                       {
                           for (unsigned int i = 0; i < frames.size(); ++i)
                           {
                               ...
                               if (equal_hist)
                               {
                                   ...
                                   PLSLAM::util::equalize_histogram(left_img);
                                   PLSLAM::util::equalize_histogram(right_img);
                               }
                               else
                               {
...
int main(int argc, char *argv[])
{
    ...
    // load configuration
    std::shared_ptr<PLSLAM::config> cfg;
    try
    {
        cfg = std::make_shared<PLSLAM::config>(config_file_path->value());
    }
    catch (const std::exception &e)
    {
        std::cerr << e.what() << std::endl;
    ...
    // run tracking
    if (cfg->camera_->setup_type_ == PLSLAM::camera::setup_type_t::Monocular)
    {
        mono_tracking(cfg, vocab_file_path->value(), data_dir_path->value(),
                      frame_skip->value(), no_sleep->is_set(), auto_term->is_set(),
                      eval_log->is_set(), map_db_path->value(), equal_hist->is_set());
    }
    else if (cfg->camera_->setup_type_ == PLSLAM::camera::setup_type_t::Stereo)
    {
